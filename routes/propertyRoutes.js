...
const { verifyFirebaseToken, requireUserInDB } = require('../middleware/firebaseAuth');
const { authorize } = require('../middleware/auth');
const { uploadPropertyFilesSingle, allInOneUpload,handlePropertyUploadErrors } = require('../middleware/uploadProperty');
...
// Image upload route (separate from create/update)
router.post('/upload-images', uploadPropertyFilesSingle, handlePropertyUploadErrors, uploadPropertyImages);
router.get('/customer', getCustomerProperties);

// Property CRUD routes with image upload
router.get('/', getProperties);
router.get('/:id', getProperty);
router.post('/', uploadPropertyFilesSingle, handlePropertyUploadErrors, createProperty);
router.put('/:id', uploadPropertyFilesSingle, handlePropertyUploadErrors, updateProperty);
router.delete('/:id', deleteProperty);
router.put('/:id/deactivate', deactivateProperty);
